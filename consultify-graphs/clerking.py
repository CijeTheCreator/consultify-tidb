...
from models import *
from prompts import *
from helpers import add_message, get_doctors_by_specialty, create_message, assign_doctor, format_conversation_history, get_user
import random
load_dotenv()
...
def translate_message(state: AgentState):
    ...
    # Use add_message to update the existing message from last_inserted_message_by_user
    message_id = state.last_inserted_message_by_user.id
    if not message_id:
        raise ValueError("No message ID found in last_inserted_message_by_user")
    
    # Update message state to show we're translating
    add_message(
        message_id=message_id,
        state="Translating"
    )
    
    # Create Message object for next_message_to_append using existing message details
    from models import Message
    next_message_to_append = Message(
        id=message_id,
        sender_id=sender_id,
    ...
    # Determine target languages using new approach
    target_language = "en"  # Default to English
    llm_language = "en"     # LLM always processes in English
    
    # Get patient and doctor information using consultation IDs
    if state.consultation.patient_id and state.consultation.doctor_id:
        try:
            patient_info = get_user(state.consultation.patient_id)
            doctor_info = get_user(state.consultation.doctor_id)
            
            # Determine who is sending the message and set target language accordingly
            if sender_id == state.consultation.patient_id:
                # Patient is sending, translate to doctor's language
                target_language = doctor_info.get('language', 'en')
            elif sender_id == state.consultation.doctor_id:
                # Doctor is sending, translate to patient's language
                target_language = patient_info.get('language', 'en')
