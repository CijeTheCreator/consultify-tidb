...
class Doctor(BaseModel):
    id: Optional[str] = Field(default=None, description="Unique identifier for the doctor")
    language: Optional[str] = Field(default=None, description="Native language of the doctor")
    specialty: Optional[str] = Field(default=None, description="Specialty of the doctor")
...
class AgentState(BaseModel):
    ...
    medical_specialty: Optional[str] = Field(default=None, description="Medical specialty appropriate for the user")
...
class PrescriptionAgentState(BaseModel):
    ...
    context_retrieved: Optional[str] = Field(default=None, description="Context retrieved from tidb vector store")
    consultation: Optional[Consultation] = Field(default=None, description="Metadata of the consultation")
    prescriptions_recommended: Optional[List["Prescription"]] = Field(default=[], description="List of prescriptions reccomended for this consultation")
...
class GeneratedMessage(BaseModel):
    """Model for generating response messages."""
    message_content: str = Field(
        description="The generated message content for the medical intake specialist"
    )

class MedicalSpecialty(BaseModel):
    """Model for determining the required medical specialty."""
    specialty: Literal[
        "General Medicine",
        "Cardiology", 
        "Dermatology",
        ...
        description="The most appropriate medical specialty for the patient's condition"
    )

class DoctorSelectionRationale(BaseModel):
    """Model for generating doctor selection rationale."""
    rationale: str = Field(
        description="Concise rationale explaining why this doctor is the optimal choice for the patient's needs (maximum 3 sentences)"
    )

class TranslatedMessage(BaseModel):
    """Model for translated message content."""
    translated_content: str = Field(
        description="The translated message content"
    )

class PrescriptionRecommendation(BaseModel):
    """Model for prescription recommendations generated by the system."""
    recommendations: str = Field(
        description="Comprehensive prescription recommendations in paragraph format (maximum 300 words)"
    )

class PrescriptionData(BaseModel):
    """Model for structured prescription data generation."""
    drug_name: str = Field(description="Name of the prescribed drug")
    frequency: str = Field(description="Frequency of drug administration (e.g., 'twice daily', 'every 8 hours')")
    duration: str = Field(description="Duration of treatment (e.g., '7 days', '2 weeks', 'ongoing')")

class PrescriptionList(BaseModel):
    """Model for a list of structured prescriptions."""
    prescriptions: List[PrescriptionData] = Field(
        description="List of structured prescription data including drug name, frequency, and duration"
    )
